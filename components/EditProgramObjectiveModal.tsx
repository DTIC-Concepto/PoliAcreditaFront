...
import {
  CreateProgramObjectiveRequest,
  ProgramObjective,
  ProgramObjectivesService,
} from "@/lib/program-objectives";
import NotificationService from "@/lib/notifications";

interface EditProgramObjectiveModalProps {
  isOpen: boolean;
...
export default function EditProgramObjectiveModal({
  isOpen,
  objective,
  onClose,
  onObjectiveUpdated,
}: EditProgramObjectiveModalProps) {
  ...
  const handleSubmit = async () => {
    if (!objective?.id) {
      NotificationService.error("Error", "No se encontró el ID del objetivo.");
      return;
    }
    ...
    try {
      ...
      const payload: CreateProgramObjectiveRequest = {
        ...formData,
        carreraId,
      };

      const errors = ProgramObjectivesService.validateObjective(payload);
      if (errors.length > 0) {
        NotificationService.warning("Datos inválidos", errors.join(", "));
        return;
      }

      await ProgramObjectivesService.updateProgramObjective(objective.id, payload);

      NotificationService.success(
        "Objetivo actualizado",
        `El objetivo ${formData.codigo} ha sido actualizado exitosamente.`
      );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-semibold text-gray-900 font-montserrat">
              Editar Objetivo de Programa
            </h2>
            <p className="text-sm text-gray-500 mt-1 font-open-sans">
              Actualiza el código y la descripción del objetivo.
            </p>
          </div>
          <button
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200">
          ...
          >
            {isLoading ? "Actualizando..." : "Actualizar Objetivo"}
          </button>
        </div>
      </div>
    </div>
